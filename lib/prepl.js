...
function Prepl(options) {
    ...
    server = net.createServer(function replOnConnection (socket) {
        ...
        socket.on('data', function socketOnData(chunk) {
            ...
            // function called when a command is done
            function done () {
                ready = true;
                socket.write(prompt);
            }
            ...
            if (isBuiltin(name)) {
                ...
                ready = false;
                // pass it the socket and what to do when its finished
                builtin.actions[name](socket, done);
                if (name === 'exit') {
                    return true;
                }
            }
            ...
            // or, if its a custom command
                ...
                ready = false;
                commands.actions[name](socket, done);
            } 
            
            // of course, if its nothing, just do nothing and give them their prompt
            else if (name === '') {
                done();
            } 
